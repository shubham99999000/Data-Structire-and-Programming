...
CirMgr::fileSim(ifstream& patternFile)
{
   ...
   for(int i = 0; i < fileStr.size(); i ++){
   	...
   	// add nonDfsList CONST0 (if needed)
   	int hashCount = 0;
		if(i == 0){			
   		if(addNonDfsConst()) hashCount = 1;
   	}	
...
bool
CirMgr::addNonDfsConst(){
	if(getRepSim()) return false;
	bool constAdd = true;
	for(int j = 0, n = _dfsList.size(); j < n; j++){
		if(_dfsList[j]->getGatetype() == CONST_GATE) constAdd = false;
	}
	if(constAdd){
		CirGate* g0 = getGate(0);
		FECGroup* f = new FECGroup(0, 0);
		_simHach.push_back(f);	
		_simHach[0]->insert(g0);
		g0->setBukSl(0, 0);			
	}
	return constAdd;
}
