...
CirMgr::readCircuit(const string& fileName)
{
   ...
   for(i = 1; i < 1 + synt[1]; i++){
      ...
      CirGate* temp;
      if(Num != 0)  temp = new CirGate(unsigned(Num/2), PI_GATE, fileStr[i], i);    // gateId, getType, lineNum, lineNo(start from zero)
      else          temp = new CirGate(unsigned(Num/2), CONST_GATE, fileStr[i], unsigned(i));
   ...
   for(i = 0; i < synt[3]; i++){
      ...
      CirGate* temp = new CirGate(unsigned(1 + i + synt[0]), PO_GATE, fileStr[i+lineNo], unsigned(i + lineNo));
      _totalList.push_back(temp);
   ...
   for(; lineNo < (j+synt[4]); lineNo++){
      ...
      CirGate* temp = new CirGate(unsigned(tempNum/2), AIG_GATE, fileStr[lineNo], unsigned(lineNo));
      _totalList.push_back(temp);
   }
   ...
   for(i = 0; i < _totalList.size(); i++){
      if(_totalList[i]->getGatetype() == PO_GATE){
         ...
         if(g == 0){
            if(Num/2 == 0){
               g = new CirGate(0, CONST_GATE, "CONST", 0);      // gateId, getType, lineNum, lineNo(start from zero)
            ...
            // undef
               ...
               g = getUndef(Num/2);
               if(g == 0){
                  g = new CirGate(unsigned(Num/2), UNDEF_GATE, "UNDEF", 0); 
...
CirMgr::printSummary() const
{
   ...
   cout << "  AIG  " << setw(9) << right << synt[4] << "  " << endl;
   cout << "------------------" << endl; 
   cout << "  Total" << setw(9) << right << (synt[1] + synt[3] + synt[4]) << "  " << endl;
}
...
CirMgr::printFloatGates() const
{
   ...
   for(i = 1; i <= _totalList.size(); i++){
      ...
      if(g!= PO_GATE && g != UNDEF_GATE && _totalList[i-1]->getFanOutSize() == 0){
         add = true;
