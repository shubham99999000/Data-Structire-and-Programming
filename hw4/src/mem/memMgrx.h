...
class MemRecycleList
{
   ...
   void  pushFront(T* p) {
      ...
      if(_first != 0) (*b) = (size_t)_first;                                        // diff
      else *b = 0;
      _first = p;
      //cout << _first;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      MemBlock<T>* _temp = _activeBlock->getNextBlock();
      while(_temp!=0){
         delete _activeBlock;
         _activeBlock = _temp;
      ...
      if(b!=0){
         _blockSize = b;
         delete _activeBlock;
         MemBlock<T>* temp = new MemBlock<T>(0, b);               // diff
         _activeBlock = temp;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      size_t si = _recycleList[m].getArrSize();
      #ifdef MEM_DEBUG
      cout << m << " " << si << " " << n; 
      #endif

      MemRecycleList<T>* nxList = _recycleList[m].getNextList();
      MemRecycleList<T>* curList = &(_recycleList[m]);
      while(si != n){
         if(nxList == 0){
            MemRecycleList<T>* newRecList = new MemRecycleList<T>(n);
            curList->setNextList(newRecList);                                       //diff
            curList = newRecList;
         }
         else{
            curList = nxList;
            nxList = nxList->getNextList();
            si = curList->getArrSize();
         }
      }
      #ifdef MEM_DEBUG
      cout << "current list is : " << curList << endl;
      #endif
      return curList;
   ...
   T* getMem(size_t t) {
      ...
      // check recycle list
      MemRecycleList<T>* _mmlist = getMemRecycleList(getArraySize(t));
      ret = _mmlist->popFront();                                                  // diff
      size_t n = _mmlist->getArrSize();
      #ifdef MEM_DEBUG
      cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
      #endif

      if(ret == 0){                                                              // use memBlock
         if(!_activeBlock->getMem(t, ret)){
            #ifdef MEM_DEBUG
            cout << "start recycling" << endl;
            #endif
            ...
            _activeBlock = new MemBlock<T>(_temp, _blockSize);
   ...
   size_t getNumBlocks() const {
      // TODO
      MemBlock<T>* _nxblk = _activeBlock->getNextBlock();
      size_t i = 1;
      while(_nxblk!=0){
         i++;
         _nxblk = _nxblk->getNextBlock();
      }
      return i;
   }
