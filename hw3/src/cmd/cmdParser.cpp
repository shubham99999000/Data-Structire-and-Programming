...
{
   // TODO...
   // false : can't open files or typo
   ifstream* _tmpDf;
   _tmpDf = new ifstream(dof.c_str());
   if(*(_tmpDf)){
      if(_dofile)
...
{
   // TODO...
   map<const string, CmdExec*>::const_iterator iter;                             // still can't figure out why I should use const_iterator
...
CmdParser::parseCmd(string& option)
{
   ...
   // -------- not sure if I should save space before 2nd word in "option" -----------------
...
CmdParser::listCmd(const string& str)
{
   ...
   for(int i = 0; i < str.size(); i++){
      if(str[i] != ' ')
         onlySpace = false;                                                               // Still include 'list all situation, don't forget
   }
   // Condition 1
   if(onlySpace){
      cout << endl;
      int i = 0;
      map<const string, CmdExec*>::const_iterator iter;                        
      for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
         i++;
         string str1 = iter->first;
         string option = (iter->second)->getOptCmd(); 
         str1.append(option);
         cout << setw(12) << left << str1;

         if(i == 5) {i = 0; cout << endl;}
      }
      reprintCmd();    
   }
   // Condition 2~6
   else{
      int n = myStrGetTok(str, token);
      // Condition 5, 6
      if(str.size() > n){
         bool beepFlag = true;
         map<const string, CmdExec*>::iterator iter;
         for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
            string cmdStr = iter->first;
            string option = (iter->second)->getOptCmd(); 
            cmdStr.append(option);
            // Condistion 5
            if(myStrNCmp(cmdStr, token, cmdStr.size()) == 0){
               cout << endl;
      ...
      // Condition 2,3,4
         ...
         for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
            ...
            for(int i = 0; i < str.size(); i++){
               if(cmdStr.size() > i)
                  fitStr.push_back(cmdStr[i]);
            }
            if(myStrNCmp(str, fitStr, str.size()) == 0){
               //cout << fitStr << " " << cmdStr;
               matchCmd.push_back(cmdStr);
            }        
         }
         //Condition 4
         if(matchCmd.size() == 0){
         ...
         //Condition 3
            ...
            for(int i = str.size(); i < matchCmd[0].size(); i++)
               insertChar(matchCmd[0][i]);
            insertChar(' ');
...
CmdParser::getCmd(string cmd)
{
   ...
   for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
      ...
      string option = (iter->second)->getOptCmd(); 
      str.append(option);
      if(myStrNCmp(str, cmd, iter->first.size()) == 0){        
         e = iter->second;
         break;
      }
   }
