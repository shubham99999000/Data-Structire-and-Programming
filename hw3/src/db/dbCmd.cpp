...
DBAppendCmd::exec(const string& option)
{
   ...
   if(myStrNCmp("-Row",options[0],2) == 0){
      ...
      //Append empty row
      else if(options[1] == "-" && options.size() == 2){
         for(int i = 0; i < dbtbl.nCols(); i++)
            rowDb.push_back(INT_MAX); 
      }

      else{
         for(int i = 0; i < dbtbl.nCols(); i++){
            // fewer data is provided
            if(2+i > options.size())                   
               rowDb.push_back(INT_MAX);
               ...
               if(options[1+i] == "-")
                  ...
                  if(!myStr2Int(options[1+i], c)){
                     cout << "\"" << options[1+i] << "\"" << " is not an integer!!" << endl;
                     return CMD_EXEC_ERROR;
                  }
                  else
                     rowDb.push_back(c);                    
      ...
      cout << "Error: Illegal option!!" << endl;
      return CMD_EXEC_ERROR;
   }
...
{  
   ...
   if(myStrNCmp("-Row",options[0],2) == 0){
      ...
      if ((!checkRowIdx(options[0], c)) || (!checkColIdx(options[1], c1)))
         return CMD_EXEC_ERROR;
      else{
         dbtbl.printData(cout, dbtbl[c][c1], true);
      }
